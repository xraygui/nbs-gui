...
One of the most flexible plans, the Step Scan plan, is shown below. This plan widget is divided into four sections, which are typical of many plans.
...
The Scan Parameters section allows the user to set parameters that are unique to the selected scan. For the Step Scan plan, these are:
...
If samples have been added to the :doc:`../samples` tab, they can be selected from the dropdown menu, which has three options:
...
* **Sample Offset:** If desired, an offset may be added to the saved position for the selected sample. This may be useful for quickly scanning multiple spots on a sample.

   * x offset: Beam inboard/outboard adjustment
   * y offset: Beam up/down adjustment
   * r offset: Beam angle adjustment

Scan Setup
~~~~~~~~~~

Scan Setup offers three generically useful options:
...
The plan appears in the queue staging area.
